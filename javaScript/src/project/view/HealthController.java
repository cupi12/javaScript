...
public class HealthController implements Initializable {
	...
	public void submit(ActionEvent actionEvent) {// 결제승인
		...
		if (txtmoney.getText() == null || txtmoney.getText().isEmpty()) {
			...
			totalMoney2 = txtcard.getText();
			txtmoney.setText("0");
			txttotal.setText(totalMoney2);

		} else if (txtcard.getText() == null || txtcard.getText().isEmpty()) {
			sum = Integer.parseInt(txtmoney.getText());
			txtcard.setText("0");
			doubleMoney = sum * 0.9;
			totalMoney = Double.toString(doubleMoney);
			txttotal.setText(totalMoney);

//			tMoney = Integer.parseInt(sum);
		}
